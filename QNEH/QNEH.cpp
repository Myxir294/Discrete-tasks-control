...
{   
    ...
    clock_t start, end; //zmienne pomocnicze do przechowywania czasu dzialania programu
...
for (int k = 0; k<121; k++)
{
	...
	output_file << endl << "For task set " << k << endl;
    input_file >> temp;
    //cout << temp << endl;
    while(temp.length() != 9)//znajdz w pliku początek wartosci dotyczacych zestawu danych
    {   
        input_file >> temp;
    }
	...
	{	
        ...
        {
            if(temp_begin == 0) {temp_begin = i-1;}
            temp_end = i;
        }
        if(tasks_weigths[i] != tasks_weigths[i-1] && temp_end != 0)
        {   
            for(int k = temp_begin; k<=temp_end; k++) 
            {
                for(int j = k+1; j<=temp_end; j++)
                {       
                    if(order[j] < order[k]) 
                    {   
    ...
    //Wypisanie wyniku - długości oraz permutacji
